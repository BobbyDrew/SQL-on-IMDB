...
where TABLE_SCHEMA ="imdb" order by table_rows;
-- or
...
-- Type your code below:
SELECT
year(date_published), COUNT(id) AS number_of_movies
FROM movie
...
select distinct genre as Genre_of_the_movies from genre;
...
select g.genre,count(m.id) as movies_count from
genre g
inner join movie m
on g.movie_id=m.id
group by genre
